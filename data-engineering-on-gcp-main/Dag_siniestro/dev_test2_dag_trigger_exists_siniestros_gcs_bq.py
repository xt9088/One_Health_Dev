...
# Función para eliminar el archivo de GCS
def delete_gcs_file(**kwargs):
    ti = kwargs['ti']
    file = ti.xcom_pull(key='file', task_ids='obtener_parametros')
    source_bucket = ti.xcom_pull(key='source_bucket', task_ids='obtener_parametros')
    source_path = ti.xcom_pull(key='source_path', task_ids='obtener_parametros')
    ...
    # Eliminar el archivo de GCS
    blob.delete()
    print(f'Deleted {source_path_2} from {source_bucket}.')
...
with DAG(
    'dev_Test2_dag_trigger_siniestros_gcs_bq',
    catchup=False,
    ...
    # Tarea para eliminar el archivo de GCS
    task_delete_gcs_file = PythonOperator(
        task_id='eliminar_archivo_gcs',
        python_callable=delete_gcs_file,
        provide_context=True,
    )
    ...
    task_load_parquet_to_bq >> task_delete_gcs_file
    task_no_file_found >> task_delete_gcs_file  # Conectar también la tarea dummy después de la carga a BigQuery
