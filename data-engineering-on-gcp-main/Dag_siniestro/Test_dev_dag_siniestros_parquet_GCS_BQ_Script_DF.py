...
def fetch_and_store_values(table, id_config_table, **kwargs):
    ...
    if result:
        ...
        kwargs['ti'].xcom_push(key='bg_name', value=result_dict['DESTINATION_BG_NAME'])
...
def load_parquet_to_bigquery(**kwargs):
    ...
    bg_name = ti.xcom_pull(key='bg_name', task_ids='obtener_parametros')
    ...
    bigquery_client = bigquery.Client(project=bg_name)
...
with DAG(
    'Test_dev_dag_siniestros_parquet_GCS_BQ_Script',
    catchup=False,
